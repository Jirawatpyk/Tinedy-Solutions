...
export function BookingCreateModal({
  ...
  // Internal state เพื่อ persist package selection และป้องกันการหายเมื่อ re-render
  const [internalPackageSelection, setInternalPackageSelection] = useState<PackageSelectionData | null>(packageSelection)
  ...
  // Sync internal state กับ prop เมื่อ modal เปิดใหม่
  useEffect(() => {
    if (isOpen && packageSelection) {
      setInternalPackageSelection(packageSelection)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (internalPackageSelection?.estimatedHours && createForm.formData.start_time) {
        calculatedEndTime = calculateEndTime(createForm.formData.start_time || '', internalPackageSelection.estimatedHours * 60)
      }
      ...
      if (enableRecurring && recurringDates.length > 0 && internalPackageSelection?.frequency) {
        // ตรวจสอบว่าเลือกวันครบตามความถี่หรือยัง
        if (recurringDates.length !== internalPackageSelection.frequency) {
          throw new Error(`Please select ${internalPackageSelection.frequency} dates for recurring bookings`)
        }
        ...
        console.log('📋 Base booking data for recurring group:', {
          ...
          total_price: baseBookingData.total_price
        })

        // Create recurring group
        const result = await createRecurringGroup({
          baseBooking: baseBookingData,
          dates: recurringDates,
          pattern: recurringPattern,
          totalOccurrences: internalPackageSelection.frequency as BookingFrequency,
        })
        ...
        toast({
          title: 'Success',
          description: `Created ${result.bookingIds.length} recurring bookings successfully!`,
        })
      } else {
  ...
  const resetForm = () => {
    ...
    setExistingCustomer(null)
    setInternalPackageSelection(null)
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2 sm:col-span-2">
              <PackageSelector
                ...
                value={internalPackageSelection}
                onChange={(selection) => {
                  console.log('📦 Package Selection Changed:', selection)

                  // Update internal state ทันที
                  setInternalPackageSelection(selection)
                  ...
                  startTransition(() => {
                    ...
                    if (selection) {
                      ...
                      if (selection.pricingModel === 'fixed') {
                        console.log('💰 Setting Fixed Price:', selection.price)
                        ...
                        formUpdates.service_package_id = '' // V1 field not used for V2
                        formUpdates.package_v2_id = selection.packageId
            ...
            {/* Recurring Bookings Checkbox - แสดงเฉพาะเมื่อมี frequency */}
            {internalPackageSelection?.frequency && (
              <div className="space-y-2 sm:col-span-2">
                <div className="flex items-center space-x-2">
                  ...
                  >
                    สร้าง Recurring Bookings ({internalPackageSelection.frequency} ครั้ง)
                  </Label>
                </div>
                <p className="text-xs text-muted-foreground pl-6">
                  สร้าง booking ทั้งหมด {internalPackageSelection.frequency} ครั้งตามความถี่ของแพ็คเก็จ
                </p>
              </div>
            )}

            {/* Booking Date - เปลี่ยนเป็น optional ถ้า enable recurring */}
            {!enableRecurring && (
              <div className="space-y-2">
            ...
            {enableRecurring && internalPackageSelection?.frequency && (
              <div className="space-y-2 sm:col-span-2">
                <RecurringScheduleSelector
                  frequency={internalPackageSelection.frequency as BookingFrequency}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (newStartTime && internalPackageSelection?.estimatedHours) {
                    const durationMinutes = Math.round(internalPackageSelection.estimatedHours * 60)
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled={!!internalPackageSelection?.estimatedHours}
              />
              <p className="text-xs text-muted-foreground">
                {internalPackageSelection?.estimatedHours
                  ? `คำนวณจากระยะเวลา ${internalPackageSelection.estimatedHours} ชม.`
              ...
              <div className="space-y-2 sm:col-span-2">
                <Button
                  ...
                  disabled={
                    ...
                    !internalPackageSelection
                  }
                >
                  <Sparkles className="h-4 w-4 mr-2 text-tinedy-blue" />
                  Check Staff Availability
                </Button>
                {((enableRecurring ? recurringDates.length === 0 : !createForm.formData.booking_date) || !createForm.formData.start_time || !internalPackageSelection) && (
