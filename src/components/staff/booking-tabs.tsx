...
export const BookingTabs = memo(function BookingTabs({
  ...
  const tabs: Tab[] = [
    { value: 'today', label: 'Today', icon: Calendar, count: todayCount, tooltip: "Today's bookings" },
    { value: 'upcoming', label: 'Upcoming', icon: Briefcase, count: upcomingCount, tooltip: 'Upcoming bookings' },
    { value: 'past', label: 'Past', icon: CheckCircle, count: pastCount, tooltip: 'Completed bookings' },
    { value: 'stats', label: 'Stats', icon: BarChart3, tooltip: 'Performance statistics' },
  ]
      ...
      <div className="px-2 tablet-landscape:px-4 sm:px-4">
        <div className="flex gap-0.5 tablet-landscape:gap-2 sm:gap-1 overflow-x-auto hide-scrollbar">
          {tabs.map((tab) => {
            ...
            return (
              <SimpleTooltip key={tab.value} content={tab.tooltip}>
                <button
                  type="button"
