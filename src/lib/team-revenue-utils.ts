 ...
 * Returns: "staff_id.eq.{userId},team_id.in.({teamIds})"
 *
 * @param userId - User ID
 * @param teamIds - Array of team IDs
 * @returns Filter condition string or null if no teams
 */
export function buildTeamFilterCondition(userId: string, teamIds: string[]): string | null {
  if (teamIds.length === 0) {
    return null
  }
