-- Fix Customer Lifetime Value to exclude archived (soft-deleted) bookings
-- Archived bookings have deleted_at IS NOT NULL and should not be counted in stats
...
SELECT
  ...
  -- Only count non-archived bookings
  COUNT(b.id) as total_bookings,

  -- Use payment_status = 'paid' for revenue (only non-archived)
  ...
  -- Last booking date: only past service dates (booking_date <= today), excluding archived
  ...
  -- Status counts (only non-archived)
...
COMMENT ON VIEW customer_lifetime_value IS 'Customer booking statistics - excludes archived/soft-deleted bookings, lifetime_value calculated from paid bookings (payment_status = paid), last_booking_date shows most recent PAST service date (excludes future bookings)';
